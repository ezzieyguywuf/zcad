...
pub const VulkanContext = struct {
    ...
    fn createPipeline(
        ...
        const pipeline_vertex_input_state_create_info = comptime blk: {
            var binding_descriptions: []const vk.VertexInputBindingDescription = &.{};
            var attribute_descriptions: []const vk.VertexInputAttributeDescription = &.{};
            if (std.mem.eql(u8, vert_fname, "line_vertex_shader")) {
                binding_descriptions = &.{Line.binding_description};
                attribute_descriptions = &Line.attribute_description;
            } else {
                binding_descriptions = &.{Vertex.binding_description};
                attribute_descriptions = &Vertex.attribute_description;
            }
            break :blk vk.PipelineVertexInputStateCreateInfo{
